...
private:
	...
	BlockBuffer<def_block_alloc_16k, 65536> bb;
public:
	char * data()
	{
		return bb.data();
...
{
	unmarshal_container(s, std::inserter(p, p.end()));
	return s;
}

template <class T>
inline const Unpack& operator >> (const Unpack& s, std::set<T>& p)
{
	unmarshal_container(s, std::inserter(p, p.end()));
	return s;
}
...
template <class T>
inline const Unpack& operator >> (const Unpack& s, std::vector<T>& p)
{
	unmarshal_container(s, std::back_inserter(p));
	return s;
}
...
{
	marshal_container(s, p);
	return s;
}

template <class T>
inline Pack& operator << (Pack& s, const std::set<T>& p)
{
	marshal_container(s, p);
	return s;
}
...
template <class T>
inline Pack& operator << (Pack& s, const std::vector<T>& p)
{
	marshal_container(s, p);
	return s;
}
